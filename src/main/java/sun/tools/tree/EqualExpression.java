...
public final class EqualExpression extends BinaryEqualityExpression {

	public EqualExpression(final long l, final Expression expression, final Expression expression1) {
		super(20, l, expression, expression1);
	}
	...
	void codeBranch(final Environment environment, final Context context, final Assembler assembler, final Label label, final boolean flag) {
		...
		case 4: // '\004'
			if (!this.right.equals(0)) {
				...
				assembler.add(this.where, flag ? 159 : 160, label, flag);
				return;
			}
			break;
		...
		case 10: // '\n'
			if (this.right.equals(0)) {
				assembler.add(this.where, flag ? 198 : 199, label, flag);
			} else {
				this.right.codeValue(environment, context, assembler);
				assembler.add(this.where, flag ? 165 : 166, label, flag);
			}
			return;
		...
		assembler.add(this.where, flag ? 153 : 154, label, flag);
	}
}
