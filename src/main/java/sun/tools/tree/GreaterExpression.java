...
public final class GreaterExpression extends BinaryCompareExpression {

	public GreaterExpression(final long l, final Expression expression, final Expression expression1) {
		super(22, l, expression, expression1);
	}
	...
	Expression simplify() {
		return this.left.isConstant() && !this.right.isConstant() ? (Expression) new LessExpression(this.where, this.right, this.left) : this;
	}
	...
	void codeBranch(final Environment environment, final Context context, final Assembler assembler, final Label label, final boolean flag) {
		...
		case 4: // '\004'
			if (!this.right.equals(0)) {
				...
				assembler.add(this.where, flag ? 163 : 164, label, flag);
				return;
			}
			break;
		...
		assembler.add(this.where, flag ? 157 : 158, label, flag);
	}
}
