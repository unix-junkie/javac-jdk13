...
public final class LessExpression extends BinaryCompareExpression {

	public LessExpression(final long l, final Expression expression, final Expression expression1) {
		super(24, l, expression, expression1);
	}
	...
	Expression simplify() {
		return this.left.isConstant() && !this.right.isConstant() ? (Expression) new GreaterExpression(this.where, this.right, this.left) : this;
	}
	...
	void codeBranch(final Environment environment, final Context context, final Assembler assembler, final Label label, final boolean flag) {
		...
		case 4: // '\004'
			if (!this.right.equals(0)) {
				...
				assembler.add(this.where, flag ? 161 : 162, label, flag);
				return;
			}
			break;
		...
		assembler.add(this.where, flag ? 155 : 156, label, flag);
	}
}
